...
x=0.5:dx:xm;                           %横坐标向量
u=tan(x/2);                            %中间变量
%i1=2/(1+b)/c*atan(c*u);                %用反正切函数表示的积分
i1=2/c*atan(c*u/(1+b));                %用反正切函数表示的积分
c=c*i                                  %综合常数乘以虚数单位
%i2=1/(1+b)/c*log((c*u+1)./(c*u-1));    %用对数函数表示的积分
i2=log((c*u/(1+b)+1)./(c*u/(1+b)-1))/c;%用对数函数表示的积分
y=1./(1+b*cos(x));                     %被积函数
...
title('1/(\ita+b\rmcos\itx\rm)的两个不定积分公式的比较',...
    'FontSize',16)                     %加标题
