%曲线与横轴之间的平面绕横轴旋转的体积和表面积的函数文件
function fun(z,xm,tit)
syms x                                 %定义符号变量
dvx=pi*z^2;                            %绕x轴旋转的圆的面积
vx=int(dvx,0,x)                        %绕x轴旋转体的体积
% dz=diff(z)                             %求高坐标的微分
% dl=sqrt(1+dz^2)                        %弧的微分
% dl=simplify(dl)                        %化简符号被积函数
% dsx=2*pi*z*dl;                         %绕x旋转的环面的微分
% sx=int(dsx,0,x)                        %绕x轴旋转面的表面积
% fsx=inline(sx);                        %表面积的内线函数

f=inline(z);                           %母线转化为内线函数
fvx=inline(vx);                        %体积的内线函数
dx=0.01;                               %横坐标间隔
x=0:dx:xm;                             %横坐标向量
z=f(x);                                %纵坐标向量即母线向量
vx=fvx(x);                             %体积向量
...
dsx=2*pi*z.*dl;                        %绕x旋转的环面的微分
sx=cumtrapz(dsx)*dx;                   %用梯形法求面积
figure                                 %创建图形窗口
plot(x,z,x,vx,'--',x,sx,'-.','LineWidth',2)%画母线,体积和表面积
fs=20;                                 %字体大小
title([tit,'与横轴之间的平面绕横轴旋转的体积和表面积'],'FontSize',fs)%加标题
xlabel('\itx/a','FontSize',fs)         %加横坐标
grid on                                %加网格
legend('\itz/a','\itV_x/a\rm^3','\itS_x/a\rm^2',2)%加图例

[X,Y,Z]=cylinder(z);                   %取三维曲面坐标
Z=Z*xm;                                %放大竖坐标
figure                                 %创建图形窗口
surf(Z,Y,X)                            %画旋转面
...
for i=1:length(p)                      %按圆周角循环
    t=p(i);                            %取圆周角
    zz=[z,0,0]*cos(t);                 %旋转竖坐标
    yy=[z,0,0]*sin(t);                 %旋转纵坐标
