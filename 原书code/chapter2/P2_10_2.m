...
title(['双根函数的极限(\itn\rm=',num2str(n),')'],'FontSize',16)%加标题
xlabel('\itx','FontSize',16)           %加横坐标
ylabel('\itf_n_m','FontSize',16)       %加纵坐标
legend([repmat('\itm\rm=',length(m),1),num2str(m')],4)%加图例
grid on                                %加网格
ll=subs(l,{'m','n'},{m,n});            %将符号替换为整数向量
hold on                                %保持图像
plot(x0,ll,'or','LineWidth',2)         %画极限点
text(x0*ones(size(m)),ll,num2str(ll'),'FontSize',16)%标记极限值
axis([0,xm,-4,4])                      %设置曲线范围

figure                                 %创建图形窗口
plot(x,fnm(n,M,X),'LineWidth',2)       %画曲线族
title(['双根函数的极限(\itm\rm=',num2str(n),')'],'FontSize',16)%加标题
xlabel('\itx','FontSize',16)           %加横坐标
ylabel('\itf_n_m','FontSize',16)       %加纵坐标
legend([repmat('\itn\rm=',length(m),1),num2str(m')],4)%加图例
grid on                                %加网格
